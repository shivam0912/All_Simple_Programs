...
public class OrderedLinearSearch {
    public static int sortedLinearSearch(int[] a,int size,int data){
        for(int i=0;i<=size;i++){                                        // Time Complexity: O(n)
            if (a[i] == data){                                          // Space Complexity: O(1)
                return i;
            }else if(a[i]>data){
                return -1;
            }
    ...
    public static void main(String[] args) throws IOException {
        ...
        if(sortedLinearSearch(a,n-1,data)==-1)
            System.out.println("Sorry ): Element is not available in the array:");
        else
            System.out.println("Element at the position: "+sortedLinearSearch(a,n-1,data));
    }
}
